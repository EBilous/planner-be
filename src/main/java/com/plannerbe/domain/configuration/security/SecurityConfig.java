package com.plannerbe.domain.configuration.security;
...
UserDetailsService: Створює двох користувачів "serhii" та "admin"
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        httpSecurity.
            ...
            .authorizeHttpRequests(
                (authorize) ->
                    authorize
                        .requestMatchers("/api/v1/public","/api/v1/user/sign-up").permitAll()
    ...
    public UserDetailsService userDetailsService() {
        UserDetails eugene = User.builder().username("Eugene").password(passwordEncoder().encode("Qwerty123")).roles("USER")
            .build();

        UserDetails admin = User.builder().username("admin").password(passwordEncoder().encode("admin")).roles("ADMIN")
            .build();
        return new InMemoryUserDetailsManager(eugene,admin);
    }
}
